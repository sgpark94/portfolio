$(function(){
	...
	(function(){ // WHERE IS MAP 처리

		$('.btn_map').on('click', function () { // .btn_ㅡmap이 클릭되면
			$('.btn_map').removeClass('active'); // 일단 .btn_map에 들어간 active를 다 지워주고
			$(this).addClass('active'); // 선택된 li.btn_map에 .active를 넣어줘서 css효과를 준다.
			var src = $(this).attr('data-src'); //변수 src에 .btn_map의 data-src속성을 저장
			$('.city_wrap img').attr('src', '../img/maps/'+src); // .city_wrap img의 src 속성을 this된 li.btn_map의 src에 입력
			var txt = $(this).attr('data-area'); // 변수 txt에 .btn_map의 data-area속성을 저장
			$('.map_area').text(txt);
		})
	})();
	(function(){ // 헤더 메뉴 볼드처리
		var idx = $('#container').attr('data-index');
		$('header nav a.page_'+idx).css({
